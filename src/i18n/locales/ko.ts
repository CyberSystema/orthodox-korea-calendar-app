// Typed as the exact shape of en.ts so `tsc` fails on ANY key that is missing
// from — or extra in — this file. A key present in en but missing here would
// render the raw key string ("settings.title") to users: a release blocker.
// This closes that gap with zero dependencies, inside the existing `npm run typecheck`.
type Translations = typeof import('./en').default;

const ko: Translations = {
  nav: {
    home: '홈',
    today: '오늘',
    month: '월간',
    announcements: '소식',
    eventDetail: '행사 상세',
    announcementDetail: '공지',
    settings: '설정',
  },
  announcements: {
    title: '공지사항',
    empty: '아직 공지사항이 없습니다',
    emptyHint: '본당 소식과 축일 알림이 여기에 표시됩니다.',
    loadError: '공지사항을 불러오지 못했습니다',
    retry: '다시 시도',
    viewEvent: '행사 보기',
    viewDetails: '자세히 보기',
    linkedEvents: '연결된 행사',
    eventUnavailable: '연결된 행사를 더 이상 볼 수 없습니다.',
    noticeLabel: '공지',
    audienceEn: '영어',
    audienceKo: '한국어',
    deleteTitle: '공지 삭제',
    deleteConfirm: '이 공지를 모두에게서 삭제하시겠습니까? 되돌릴 수 없습니다.',
    deleteAction: '삭제',
    deleteFailedTitle: '공지를 삭제하지 못했습니다',
  },
  offline: {
    banner:
      '오프라인 상태입니다 — 저장된 달력을 표시합니다. 행사 업데이트와 스태프 모드는 다시 연결될 때까지 일시 중지됩니다.',
  },
  common: {
    events: '행사',
    noEvents: '이 날짜에는 행사가 없습니다.',
    loading: '불러오는 중...',
    done: '완료',
    draft: '초안',
    on: '날짜',
    readings: '성경 구절',
    celebrations: '축일',
    saints: '성인',
    otherInfo: '기타 정보',
    itemReadings: '독서',
    tone: '성가 조',
    matins: '조과 복음',
    noReadings: '독서 없음',
    noSaints: '표시할 성인이 없습니다',
    noOtherInfo: '추가 정보가 없습니다',
    fast: '금식',
    cheese: '유제품 허용',
    fish: '생선 허용',
    pres: '미리 축성된 성찬예배',
    basil: '성 대 바실리오스 성찬예배',
    dl: '성찬예배',
  },
  today: {
    title: '정교회 한국 달력',
    goEvent: '첫 행사 열기',
    goSettings: '설정 열기',
    goToday: '오늘로 돌아가기',
    openSearch: '전례 검색',
    openDatePicker: '날짜로 이동',
    toggleLanguage: '영어/한국어 전환',
    searchTitle: '전례 내용 검색',
    searchPlaceholder: '축일, 성인, 독서를 검색하세요...',
    searchHint: '2자 이상 입력하면 검색됩니다.',
    noSearchResults: '일치하는 전례 항목이 없습니다.',
    jumpToDateTitle: '정확한 날짜로 이동',
    jumpToDatePlaceholder: 'YYYY-MM-DD',
    jumpButton: '날짜 이동',
    cancel: '닫기',
    invalidDate: 'YYYY-MM-DD 형식의 올바른 날짜를 입력해 주세요.',
    todayDateLabel: '날짜',
    yearLabel: '연도',
    monthLabel: '월',
    dayLabel: '일',
    noYearData: '{{year}}년 전례 데이터가 없습니다.',
    noYearDataPublished: '{{year}}년 전례력이 아직 게시되지 않았습니다.',
    noDayData: '해당 날짜의 전례 데이터가 없습니다.',
  },
  month: {
    title: '월간 보기',
    subtitle: '날짜를 눌러 행사 보기',
    prevMonth: '이전',
    nextMonth: '다음',
    openFirstEvent: '첫 행사 열기',
    dayDetails: '일일 상세',
    closeDayDetails: '일일 상세 닫기',
    eventDate: '행사 날짜',
    addEvent: '이벤트 추가',
    editEvent: '이벤트 수정',
    deleteEvent: '삭제',
    deleteEventTitle: '이벤트 삭제',
    deleteEventConfirm: '이 사용자 정의 이벤트를 삭제하시겠습니까?',
    save: '저장',
    adminWorkflow: '관리자 이벤트 워크플로우',
    addEventOnDate: '이 날짜에 이벤트 추가',
    syncingApi: '웹 API와 동기화 중...',
    syncedApi: '웹 API와 동기화되었습니다',
    datePlaceholder: 'YYYY-MM-DD',
    titleEnPlaceholder: '제목 (영어)',
    titleKoPlaceholder: '제목 (한국어)',
    summaryEnPlaceholder: '요약 (영어)',
    summaryKoPlaceholder: '요약 (한국어)',
    detailsEnPlaceholder: '상세 (영어)',
    detailsKoPlaceholder: '상세 (한국어)',
    eventTitleRequiredTitle: '제목 필요',
    eventTitleRequiredOneLanguage: '영어 또는 한국어 제목을 입력해 주세요.',
    eventSaveError: '이벤트 저장 실패',
    sendNotification: '알림 보내기',
    sendNotificationOnEdit: '이 변경 사항에 대한 알림 보내기',
    notifyAudience: {
      title: '받는 대상',
      all: '모두',
      english: '영어만',
      korean: '한국어만',
      hint: '각 구독자는 자신의 언어로 받습니다.',
    },
    notifyTitleRequiredTitle: '두 제목 모두 필요',
    notifyTitleRequiredBoth: '모두에게 알리려면 영어와 한국어 제목이 모두 필요합니다.',
    notifyTitleRequiredEnglish: '영어 구독자에게 알리려면 영어 제목이 필요합니다.',
    notifyTitleRequiredKorean: '한국어 구독자에게 알리려면 한국어 제목이 필요합니다.',
    recurrence: {
      title: '반복',
      none: '없음',
      daily: '매일',
      weekly: '매주',
      monthly: '매월',
    },
    weekdays: ['일', '월', '화', '수', '목', '금', '토'],
  },
  event: {
    title: '행사 상세',
    subtitle: '딥링크를 지원하는 상세 화면입니다.',
    rank: '등급',
    recurrence: '반복',
    date: '날짜',
    notFound: '행사를 찾을 수 없습니다.',
    addToCalendar: '캘린더에 추가',
    googleCalendar: '구글',
    appleCalendar: '애플',
    outlookCalendar: '아웃룩',
  },
  settings: {
    title: '설정',
    subtitle: '언어, 알림, 앱 환경설정',
    adminMode: '스태프 모드',
    fontSize: '글자 크기',
    fontSizeNormal: '보통',
    fontSizeLarge: '크게',
    fontSizeLarger: '더 크게',
    fontSizeLargest: '가장 크게',
    fontSizePreview: '거룩한 교부들의 주일 — 제6조',
    fontSizeHint:
      '기기의 글자 크기 설정에 더해서 적용되므로, 앱 글자를 시스템 설정보다 더 크게 볼 수 있습니다.',
    launchScreen: '시작 화면',
    launchScreenHint: '앱을 열었을 때 처음 보이는 화면을 선택하세요. 다음에 열 때부터 적용됩니다.',
    theme: '화면 모드',
    themeAuto: '자동',
    themeLight: '밝게',
    themeDark: '어둡게',
    themeHint: '자동을 선택하면 기기 설정을 따릅니다.',
    direction: '테마',
    directionGilded: '금빛',
    directionElegant: '우아',
    directionHint: '금빛은 채색 필사본 느낌이고, 우아는 더 차분합니다.',
    groupReading: '읽기',
    groupBehaviour: '동작',
    notificationsOpen: '기기 설정 열기',
    notifications: '알림',
    notificationsHint: '축일과 일정 알림은 기기 설정에서 켜거나 끌 수 있습니다.',
    staffEntry: '교구 담당자',
    staffIntro:
      '행사를 추가하거나 수정하는 교구 담당자를 위한 기능입니다. 담당자 암호가 필요합니다.',
    staffUnavailable: '지금은 담당자 로그인을 사용할 수 없습니다. 잠시 후 다시 시도해 주세요.',
    diagnosticsEntry: '진단 정보',
    adminPasscodePlaceholder: '스태프 비밀번호',
    saveAdminPasscode: '스태프 비밀번호 저장',
    stuffPasscodeStored:
      '스태프 세션이 활성화되어 안전하게 저장되었습니다. 스태프 모드를 끄면 로그아웃됩니다.',
    sessionExpired: '스태프 세션이 만료되었습니다. 계속하려면 비밀번호를 입력하세요.',
    passcodeCleared: '관리자 비밀번호가 삭제되었습니다.',
    passcodeSaved: '관리자 비밀번호가 검증되어 저장되었습니다.',
    passcodeFailed: '관리자 비밀번호 검증에 실패했습니다.',
    passcodeRateLimited: '로그인 시도가 너무 많습니다. 잠시 후 다시 시도하세요.',
    passcodeRateLimitedWithRetry: '로그인 시도가 너무 많습니다. {{seconds}}초 후 다시 시도하세요.',
    passcodeRequired: '스태프 모드를 활성화하려면 스태프 비밀번호를 설정하고 검증하세요.',
    adminEnabled: '스태프 모드가 활성화되고 동기화되었습니다.',
    version: '버전 {{version}}',
    offlineStaffDisabled:
      '스태프 모드는 인터넷 연결이 필요합니다. 다시 온라인 상태가 되면 자동으로 재개됩니다.',
  },
  a11y: {
    openSettings: '설정 열기',
    search: '전례 검색',
    previousDay: '이전 날',
    nextDay: '다음 날',
    previousMonth: '이전 달',
    nextMonth: '다음 달',
    decreaseValue: '감소',
    increaseValue: '증가',
    brandTitle: '정교회 한국 달력',
  },
  secret: {
    accessTitle: '시스템 접근',
    accessPrompt: '백엔드 관리자 비밀번호를 입력하세요',
    accessPlaceholder: '관리자 비밀번호',
    accessSubmit: '잠금 해제',
    accessDeniedTitle: '접근 거부됨',
    invalidPasscode: '잘못된 비밀번호입니다.',
  },
};

export default ko;
